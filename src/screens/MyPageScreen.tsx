...
function computeMaintenance(lastVisitDate: string | null): MaintenanceInfo | null {
  ...
  if (daysRemaining >= 10) {
    color    = '#78C050'
    headline = `次回推奨日まで ${daysRemaining}日`
    badge    = '快調'
  } else if (daysRemaining >= 4) {
    color    = '#F0B840'
    headline = `メンテナンス推奨まであと ${daysRemaining}日`
    badge    = 'そろそろ'
  } else if (daysRemaining >= 1) {
    color    = '#E06060'
    headline = `男前崩壊まであと ${daysRemaining}日`
    badge    = '緊急'
  } else if (daysRemaining === 0) {
    color    = '#E03030'
    headline = '今日が推奨メンテナンス日です'
    badge    = '今すぐ'
  } else {
    color    = '#E03030'
    headline = `男前崩壊 ${Math.abs(daysRemaining)}日超過`
    badge    = '崩壊'
  }
...
async function fetchUserLastVisitDate(userId: string): Promise<string | null> {
  ...
  try {
    const stored = JSON.parse(
      localStorage.getItem(MAINTENANCE_LOCAL_KEY) ?? '{}'
...
export function MyPageScreen({ memberStatus, onMemberStatusChange }: Props) {
  ...
  // undefined = loading, null = no data, string = ISO date
  ...
  async function handleInitiateGift(ticket: TicketRow) {
    ...
    try {
      ...
      setGiftModalTicket(prev =>
        prev ? { ...prev, pending_transfer: true, transfer_token: token } : prev
      )
      setTickets(prev =>
        prev.map(t =>
  ...
  async function handleCancelGift(ticket: TicketRow) {
    try {
      ...
      setTransferToken(null)
      setGiftModalTicket(prev =>
        prev ? { ...prev, pending_transfer: false, transfer_token: null } : prev
      )
      setTickets(prev =>
        prev.map(t =>
  ...
  const maintenanceInfo      = isMaintenanceLoading ? null : computeMaintenance(lastVisitDate)
  ...
  return (
    <>
      ...
      {/* ════════════════════════════════════════════════════════════════
          使用する modal — shows ticket + QR for store to scan
      ════════════════════════════════════════════════════════════════ */}
      <AnimatePresence>
        {useModalTicket && (() => {
          const t  = useModalTicket
          const tc = TICKET_TYPE_COLORS[t.type]
          const useQr = JSON.stringify({ type: 'ticket-use', ticketId: t.id, userId })
          return (
            <motion.div
              key="use-modal"
              initial={{ opacity: 0 }}
              ...
              onClick={() => setUseModalTicket(null)}
            >
              <motion.div
              ...
              >
                ...
                <div style={{
                  ...
                  background: 'linear-gradient(155deg, #160B07 0%, #0A0504 100%)',
                  border: `1px solid ${tc.border}`,
                  boxShadow: `0 24px 56px rgba(0,0,0,0.7), 0 0 24px ${tc.border}44`,
                }}>
                  ...
                  <div style={{ padding: '20px 20px 18px' }}>
                    ...
                    <p style={{ fontFamily: SERIF, fontSize: 22, fontWeight: 700, color: '#F2E6C8', marginBottom: t.amount > 0 ? 4 : 12 }}>
                      {t.title}
                    </p>
                    {t.amount > 0 && (
                      <p style={{ fontFamily: SERIF, fontSize: 36, fontWeight: 700, color: '#C9A24A', marginBottom: 16, lineHeight: 1 }}>
                    ...
                    <div style={{ background: '#FFF', borderRadius: 12, padding: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
                      <QRCodeSVG value={useQr} size={160} level="M" />
                    </div>
            ...
            >
              <motion.div
                ...
                style={{
                  ...
                  border: '1px solid rgba(201,162,74,0.24)', borderBottom: 'none',
              ...
              >
                ...
                <div style={{ padding: '12px 14px', borderRadius: 12, background: tc.bg, border: `1px solid ${tc.border}`, marginBottom: 18, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div>
                    <p style={{ fontSize: 9, fontWeight: 700, color: tc.text, letterSpacing: '0.1em', marginBottom: 3 }}>
                      {TICKET_TYPE_LABELS[t.type]}
                    </p>
                    <p style={{ fontFamily: SERIF, fontSize: 15, fontWeight: 700, color: '#F2E6C8' }}>{t.title}</p>
                  </div>
                  {t.amount > 0 && (
                    <p style={{ fontFamily: SERIF, fontSize: 20, fontWeight: 700, color: '#C9A24A' }}>¥{t.amount.toLocaleString()}</p>
                  )}
                </div>
                ...
                {!isPending && !transferUrl ? (
                  ...
                  <>
                    <p style={{ fontSize: 11, color: 'rgba(242,230,200,0.36)', lineHeight: 1.7, marginBottom: 18, textAlign: 'center' }}>
                      QRコードまたはリンクで友人に贈れます。<br />
                      相手のチケットウォレットに追加されます。
                    </p>
                    <button
                      type="button"
                      onClick={() => void handleInitiateGift(t)}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {transferUrl && (
                        <>
                          <button
                            type="button"
                            onClick={() => void handleShareUrl(transferUrl)}
                          ...
                          <button
                            type="button"
                            onClick={() => { window.open(`https://line.me/R/msg/text/?${encodeURIComponent(transferUrl)}`, '_blank') }}
                      ...
                      <button
                        type="button"
                        onClick={() => void handleCancelGift(t)}
      ...
      <div>
        ...
        <div style={{ padding: '40px 24px 20px', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: 9, letterSpacing: '0.38em', color: 'rgba(201,162,74,0.38)', marginBottom: 7, textTransform: 'uppercase' }}>
            Ginjiro Official
          </p>
          <h1 style={{ fontFamily: SERIF, fontSize: 36, fontWeight: 700, color: '#F2E6C8', letterSpacing: '0.1em', margin: '0 0 4px', lineHeight: 1, textShadow: '0 0 30px rgba(201,162,74,0.15)' }}>
            銀二郎
          </h1>
          <p style={{ fontSize: 8, letterSpacing: '0.3em', color: 'rgba(201,162,74,0.26)', textTransform: 'uppercase' }}>
            Otoko-Mae Passport
          </p>
        ...
        <motion.div
          ...
          style={{
            margin: '0 16px', borderRadius: 22, overflow: 'hidden', position: 'relative',
            ...
            boxShadow: '0 28px 60px rgba(0,0,0,0.82), inset 0 1px 0 rgba(242,230,200,0.09)',
          }}
        >
          ...
          <div style={{ padding: '22px 22px 20px' }}>
            <p style={{ fontSize: 8, letterSpacing: '0.3em', color: 'rgba(201,162,74,0.36)', textTransform: 'uppercase', marginBottom: 14 }}>
            ...
            <p style={{ fontFamily: 'monospace', fontSize: 10, color: 'rgba(242,230,200,0.3)', letterSpacing: '0.06em', wordBreak: 'break-all', marginBottom: 18 }}>
              {userId}
            </p>
            <button
              type="button"
        ...
        <div style={{ padding: '28px 16px 0' }}>
          ...
          {isMaintenanceLoading ? (
            <div style={{ borderRadius: 18, border: '1px solid rgba(255,255,255,0.07)', background: 'rgba(255,255,255,0.01)', padding: '24px', textAlign: 'center' }}>
            ...
            <div style={{ borderRadius: 18, background: 'linear-gradient(155deg, #130A07 0%, #0A0504 100%)', border: '1px solid rgba(255,255,255,0.09)', padding: '24px 20px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 11, color: 'rgba(242,230,200,0.2)', lineHeight: 1.7, marginBottom: 18 }}>
                まずは一度ご来店ください。<br />
                スタッフが来店日を記録します。
              </p>
              <button
                type="button"
                onClick={() => window.open(RESERVATION_URL, '_blank')}
            ...
            <div style={{ borderRadius: 18, overflow: 'hidden', background: 'linear-gradient(155deg, #130A07 0%, #0A0504 100%)', border: `1px solid ${maintenanceInfo.color}30`, boxShadow: `0 0 40px ${maintenanceInfo.color}0B` }}>
              ...
              <div style={{ padding: '18px 20px 20px' }}>
                ...
                <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
                  {[
                    { label: '前回来店日', value: fmtDate(maintenanceInfo.lastVisitDate) },
                    { label: '次回推奨日', value: fmtDate(maintenanceInfo.nextDate) },
                  ].map((item, i) => (
        ...
        <div style={{ paddingTop: 28 }}>
              ...
              {allActiveTickets.map(ticket => {
                ...
                return (
                  <div
                    key={ticket.id}
                    style={{
                      minWidth: 252, maxWidth: 272, flexShrink: 0,
                      scrollSnapAlign: 'start',
                      borderRadius: 18, overflow: 'hidden',
                      background: 'linear-gradient(155deg, #160B07 0%, #0A0504 100%)',
                      border: `1px solid ${isPending ? 'rgba(255,180,0,0.34)' : isExpired ? 'rgba(255,255,255,0.06)' : tc.border}`,
                      boxShadow: '0 12px 32px rgba(0,0,0,0.52)',
                    }}
                  >
                    {/* Type accent line */}
                    ...
                    <div style={{ padding: '16px 16px 14px' }}>
                      ...
                      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 10 }}>
                        <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 99, background: tc.bg, border: `1px solid ${tc.border}`, color: tc.text, letterSpacing: '0.1em' }}>
                          {TICKET_TYPE_LABELS[ticket.type]}
                        </span>
                        {isPending && (
                          <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 99, background: 'rgba(255,180,0,0.1)', border: '1px solid rgba(255,180,0,0.38)', color: '#FFB400' }}>贈り中</span>
                        )}
                        {isExpired && (
                          <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 99, background: 'rgba(224,96,80,0.1)', border: '1px solid rgba(224,96,80,0.34)', color: '#E06050' }}>期限切れ</span>
                        )}
                        {!isPending && !isExpired && (
                          <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 7px', borderRadius: 99, background: 'rgba(120,192,80,0.1)', border: '1px solid rgba(120,192,80,0.28)', color: '#78C050' }}>未使用</span>
                        )}
                      </div>

                      {/* Title */}
                      <p style={{ fontFamily: SERIF, fontSize: 17, fontWeight: 700, color: '#F2E6C8', letterSpacing: '0.03em', marginBottom: ticket.amount > 0 ? 2 : 10 }}>
                        {ticket.title}
                      </p>

                      {/* Amount */}
                      {ticket.amount > 0 && (
                        <p style={{ fontFamily: SERIF, fontSize: 28, fontWeight: 700, color: '#C9A24A', letterSpacing: '0.02em', marginBottom: 8, lineHeight: 1 }}>
                      ...
                      <p style={{ fontSize: 9, color: isExpired ? '#E06050' : 'rgba(242,230,200,0.26)', letterSpacing: '0.06em', marginBottom: 14 }}>
                        {ticket.expires_at
                          ? `期限 ${fmtDate(ticket.expires_at)}`
                          : `発行 ${fmtDate(ticket.created_at)}`}
                      </p>
                      ...
                      <div style={{ display: 'flex', gap: 7 }}>
                        ...
                        <button
                          type="button"
                          onClick={() => setUseModalTicket(ticket)}
                          disabled={isExpired}
          ...
          {usedCount > 0 && (
            <p style={{ fontSize: 9, color: 'rgba(242,230,200,0.17)', marginTop: 8, textAlign: 'center', letterSpacing: '0.1em' }}>
              使用済み {usedCount}件
            </p>
          )}
        </div>
        ...
        <div style={{ margin: '28px 16px 0' }}>
          <p style={{ fontSize: 8, letterSpacing: '0.34em', color: 'rgba(201,162,74,0.36)', textTransform: 'uppercase', marginBottom: 10 }}>
            来店スタンプ
          </p>
          <div style={{ borderRadius: 20, background: 'radial-gradient(circle at 50% -12%, rgba(139,26,42,0.16), transparent 55%), linear-gradient(160deg, #130A07 0%, #0A0504 100%)', border: '1px solid rgba(201,162,74,0.2)', padding: '20px 20px 16px' }}>
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          background: filled ? 'radial-gradient(circle at 36% 30%, #F0D060, #9A7418)' : 'rgba(255,255,255,0.03)',
                          border: filled ? '1px solid rgba(232,197,71,0.36)' : '1px solid rgba(201,162,74,0.1)',
                          boxShadow: filled ? '0 0 16px rgba(201,162,39,0.42), inset 0 1px 0 rgba(255,255,255,0.24)' : 'none',
            ...
            <p style={{ textAlign: 'center', fontFamily: SERIF }}>
              <span style={{ fontSize: 26, fontWeight: 700, color: memberStatus.stampCount > 0 ? '#E8C547' : 'rgba(242,230,200,0.22)' }}>
            ...
            {stampRewardMsg && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                style={{ marginTop: 12, padding: '10px 16px', borderRadius: 12, background: 'rgba(120,192,80,0.1)', border: '1px solid rgba(120,192,80,0.3)', textAlign: 'center' }}
              >
            ...
            <button
              type="button"
              onClick={() => {
                const next = { ...memberStatus, stampCount: 10 }
                onMemberStatusChange(next)
                saveMemberStatus(next)
              }}
